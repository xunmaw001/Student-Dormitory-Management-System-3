<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.wupinleixing" type="text" placeholder="物品类型" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                          <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.wupintupian?baseUrl+product.wupintupian.split(',')[0]:''"></image>
            ...
            <text v-if="userid && isAuth('xunwuqishi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('xunwuqishi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('xunwuqishi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('xunwuqishi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
...
<script>
	export default {
        ...
        params['order'] = 'desc';
				...
				if(this.searchForm.wupinleixing){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xunwuqishi`, params);
                } else {
                    res = await this.$api.list(`xunwuqishi`, params);
                }
				// 如果是第一页数据置空
			...
			async search(){
				...
				if(this.searchForm.wupinleixing){
					searchForm['wupinleixing'] = '%' + this.searchForm.wupinleixing + '%'
				}
				if(this.searchForm.xingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xunwuqishi`, searchForm);
                } else {
                    res = await this.$api.list(`xunwuqishi`, searchForm);
                }
				// 如果是第一页数据置空
