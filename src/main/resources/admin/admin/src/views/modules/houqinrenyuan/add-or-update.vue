...
export default {
  data() {
	...
	ruzhiriqi : false,
      ...
      rules: {
          houqinzhanghao: [
                { required: true, message: '后勤账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          houqinxingming: [
                { required: true, message: '后勤姓名不能为空', trigger: 'blur' },
          ],
          nianling: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `houqinrenyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.houqinrenyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
