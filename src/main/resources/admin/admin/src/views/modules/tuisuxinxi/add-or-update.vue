<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="退宿类型" prop="tuisuleixing">
          <el-select :disabled="ro.tuisuleixing" v-model="ruleForm.tuisuleixing" placeholder="请选择退宿类型">
            <el-option
                v-for="(item,index) in tuisuleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="退宿类型" prop="tuisuleixing">
	      <el-input v-model="ruleForm.tuisuleixing"
                placeholder="退宿类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="退宿日期" prop="tuisuriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.tuisuriqi" 
                type="date"
                :readonly="ro.tuisuriqi"
                placeholder="退宿日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tuisuriqi" label="退宿日期" prop="tuisuriqi">
              <el-input v-model="ruleForm.tuisuriqi" 
                placeholder="退宿日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="退宿说明" prop="tuisushuoming">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.tuisushuoming" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tuisushuoming" label="退宿说明" prop="tuisushuoming">
                    <span v-html="ruleForm.tuisushuoming"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	userid : false,
      ...
      rules: {
          ...
          tuisuleixing: [
                { required: true, message: '退宿类型不能为空', trigger: 'blur' },
          ],
          tuisushuoming: [
          ],
          tuisuriqi: [
                { required: true, message: '退宿日期不能为空', trigger: 'blur' },
          ],
          suguanzhanghao: [
          ],
          suguanxingming: [
  ...
  created() {
	this.ruleForm.tuisuriqi = this.getCurDate()
  ...
  methods: {
	...
	this.ruleForm.tuisushuoming = this.ruleForm.tuisushuoming.replace(reg,'../../../springbootn73cv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tuisuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tuisuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
