<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.qingkuangzhaopian" label="情况照片" prop="qingkuangzhaopian">
          <file-upload
          tip="点击上传情况照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.qingkuangzhaopian?ruleForm.qingkuangzhaopian:''"
          @change="qingkuangzhaopianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.qingkuangzhaopian" label="情况照片" prop="qingkuangzhaopian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.qingkuangzhaopian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
		...
		if(json.chuangweihao!=''&&json.chuangweihao){
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../springbootn73cv/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.qingkuangzhaopian!=null) {
		this.ruleForm.qingkuangzhaopian = this.ruleForm.qingkuangzhaopian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaisuqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaisuqingkuangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    qingkuangzhaopianUploadChange(fileUrls) {
	this.ruleForm.qingkuangzhaopian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
