<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="宿舍楼号" prop="sushelouhao">
          <el-select :disabled="ro.sushelouhao" @change="sushelouhaoChange" v-model="ruleForm.sushelouhao" placeholder="请选择宿舍楼号">
            <el-option
                v-for="(item,index) in sushelouhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.sushelouhao" label="宿舍楼号" prop="sushelouhao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.suguanxingming!=''&&json.suguanxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/sushefenpei/sushelouhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.sushelouhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    sushelouhaoChange () {
      this.$http({
        url: `follow/sushefenpei/sushelouhao?columnValue=`+ this.ruleForm.sushelouhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `sushedengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.sushedengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
